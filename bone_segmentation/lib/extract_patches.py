...
def get_data_training(xrays_train_imgs_original,
                      xrays_train_groudTruth,
                      patch_height,
                      patch_width,
                      N_subimgs):
    train_imgs_original = load_hdf5(xrays_train_imgs_original)
    train_masks = load_hdf5(xrays_train_groudTruth) #masks always the same
...
#Load the original data and return the extracted patches for training/testing
def get_data_testing(xrays_test_imgs_original, xrays_test_groudTruth, Imgs_to_test, patch_height, patch_width):
    ### test
    test_imgs_original = load_hdf5(xrays_test_imgs_original)
    test_masks = load_hdf5(xrays_test_groudTruth)
...
def get_data_testing_overlap(xrays_test_imgs_original, xrays_test_groudTruth, Imgs_to_test, patch_height, patch_width, stride_height, stride_width):
    ### test
    test_imgs_original = load_hdf5(xrays_test_imgs_original)
    test_masks = load_hdf5(xrays_test_groudTruth)
...
def extract_random(full_imgs,full_masks, patch_h,patch_w, N_patches, inside=True):
    if (N_patches%full_imgs.shape[0] != 0):
        print "N_patches: plase enter a multiple of 14"
        exit()
...
def inside_FOV(i, x, y, xrays_masks):
    assert (len(xrays_masks.shape)==4)  #4D arrays
    assert (xrays_masks.shape[1]==1)  #xrays masks is black and white
    # xrays_masks = xrays_masks/255.  #NOOO!! otherwise with float numbers takes forever!!

    if (x >= xrays_masks.shape[3] or y >= xrays_masks.shape[2]): #my image bigger than the original
        return False

    if (xrays_masks[i,0,y,x]>0):  #0==black pixels
        # print xrays_masks[i,0,y,x]  #verify it is working right
        return True
    else:
        return False
